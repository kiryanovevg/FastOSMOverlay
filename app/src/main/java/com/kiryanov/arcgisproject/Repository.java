...
public class Repository {
    ...
    public Observable<List<Polygon>> getDistricts(Context context) {
        return getGeoJson(
                Observable.fromCallable(() -> getRequest(URL_DISTRICTS)),
                context,
                1
        );
    }
    ...
    public Observable<List<Polygon>> getSettlement(Context context) {
        return getGeoJson(
                Observable.fromCallable(() -> getRequest(URL_SETTLEMENT)),
                context,
                1
        );
    }

    public Observable<List<Polygon>> getCustomPolygon(Context context, int count) {
        ...
        return getGeoJson(
                Observable.fromCallable(() -> geoJson),
                context,
                count
        );
    }

    private Observable<List<Polygon>> getGeoJson(Observable<String> source,
                                                 Context context,
                                                 int count) {

        Observable<List<Polygon>> main = source//Observable.fromCallable(() -> geoJsonStr)
                    ...
                    for (int i = 0; i < count; i++) {
                        for (JsonElement fe : features) {
                            ...
                            Polygon polygon = new Polygon(
                                    new LinearRing(geoPoints), new ArrayList<>()
                            );
