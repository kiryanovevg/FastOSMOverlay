...
public class Repository {
    ...
    public Observable<List<Polygon>> getDistricts(Context context) {
        return getPolygons(context, () -> getRequest(URL_DISTRICTS));
    }

    public Observable<List<Polygon>> getSettlement(Context context) {
        return getPolygons(context, () -> getRequest(URL_SETTLEMENT));
    }

    public Observable<List<Polygon>> getCustomPolygon(Context context) {
        ...
        return getPolygons(context, () -> geoJson);
    }

    private Observable<List<Polygon>> getPolygons(Context context, Method<String> source) {
        Observable<List<Polygon>> main = Observable.fromCallable(source::execute)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnNext(s -> Toast.makeText(context, "GeoJson loading", Toast.LENGTH_SHORT).show())
                .observeOn(Schedulers.computation())
                .map(this::parseGeoJson)
    ...
    private List<Polygon> parseGeoJson(String geoJson) {
        ...
        for (int i = 0; i < count; i++) {
            for (JsonElement fe : features) {
                ...
                LinearRing linearRing = new LinearRing(geoPoints);
                Polygon polygon = new Polygon(linearRing, new ArrayList<>());
                list.add(polygon);
            }
        }
